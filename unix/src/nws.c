...
#endif
		case N_CON_STRING:
			s_str_kid( 0, ntree, allocstring(str_kid(0,loc)) );
			break;
			}
...
nodep xwhere;		/* what stub we graft on */
{
	...
	if( strchr( Valid_Nodes[insclass], ntype(what) ) ) {
		...
		fixNames(ret, TRUE);
		block = my_block(foo);
		c_comp_decls(my_scope(block), block, TC_STUBS);
		fixNames(ret, FALSE);
		}
	 else
...
nodep _np;
{
	...
	printt2("fixDecls(%x(%s), %x)\n", np, NodeName(ntype(np)));

	if (type == N_DECL_ID)
		add_decl(np);
	else
		for (i = 0; i < nkids; i++)
			fixDecls(node_kid(np, i));

}

fixNames(_np, noProcCode)
nodep	_np;
{
	register nodep np = _np;
	register int	i;
	...
	if (type == N_NAME) {
		...
		for (i = 0; i < nkids; i++)
			if (!(noProcCode && 
			      ((type == N_DECL_FUNCTION && i == 5) ||
			       (type == N_DECL_PROCEDURE && i == 4))))
				fixNames(node_kid(np, i), noProcCode);
}
#endif PARSER
