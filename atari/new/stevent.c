...
/*
 * This is the main Gem Event handler, it takes all the gem events
 * (such as redraw events, menu selections, etc) and converts them
 * into equivalent ALICE commands, thus menu selections are just a
 * short form of entering an ALICE command.
 */
...
GemEvent()
{
	...
	/*
	 * Get the next event from GEM (mouse click, message (redraw, etc),
	 * or a keystroke)
	 */
	...
	/*
	 * If we got a mouse click, go handle it
	 */
	...
	/*
	 * If we got a rectangle event (mouse crossed the boundary of the
	 * specified box), then handle it.
	 * This is not currently used in ST Alice, but could allow you to
	 * say change the mouse shape depending on where it is.
	 */
	if( ev_which & MU_M1 ) {
	...
	/*
	 * Keep on processing events until we want to actually process
	 * a command.
	 */
	} while ( !done );
...
hndl_button()
{
	...
	/*
	 * If the window we clicked in is an edit window...
	 */
	if( EditWin( handle ) ) {
		/*
...
/*
 * Handle the mouse crossing the M1 rectangle - NOTE: Not currently used
 */
hndl_mouse()
{
	/*
...
/*
 * We got a message event, go handle it
 */
int
hndl_msg()
{
...
do_gdupdate()
{
	menu_icheck( gl_menu, DMDEBUG, work_debug( curr_workspace ) & DBF_ON ?
		1 : 0 );
	menu_icheck( gl_menu, DMCURFOL, work_debug(curr_workspace) & DBF_CURSOR
		 ? 1 : 0 );
}
do_gkeys()
{
