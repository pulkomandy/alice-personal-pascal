...
/*
 *
 *  Routines to deal with workspaces - special named subtrees
...
#ifdef GEM
int	DbFlag;		/* Global Debug status */
#endif
...
#ifdef HAS_LIBRARY
		case N_LIBRARY:
			/* copy library file name */
			s_str_kid(2, ntree, allocESString(getESString(
				str_kid(2, loc))));
			/* If this is a binary library, copy the
			 * node block
			 */
			if( kid4(loc) ) {
				extern nodep libcopy();
				s_kid4( ntree, libcopy( kid4(loc) ));
			} else
				s_kid4( ntree, NIL );
...
char *args;
{
	...
	if (args ||
#ifdef GEM
	( form_alert( 1, LDS( 479,
   "[2][Clearing a workspace|cannot be undone|Do it anyway?][Yes|No]")) == 1 )
#else
        !com_menu(M_clearws, TRUE)
#endif
	) {
...
#ifdef GEM
		if( strcmp( work_name( tospace ), "clipboard" ) != 0 )
#endif
...
int getwhat;		/* what ws to get.  -1 for all, WS_FRAGMENT for frags*/
{
...
#ifdef GEM
		pop_menu( &m, (funcptr)0, FALSE, &str, (int *)0 );
		/* User cancelled the operation */
		if( str == (char *)0 )
			err_return();
		/* Start off with a nil */
		prename = (char *)0;
		/* If we are not creating a new workspace... 
		 * or he did not select 'new workspace' */
		if( !cflag || !(str == nws) )
			prename = str;
#else
	...
	if( !prename  ) {
		getprline( LDS( 481, "Workspace Name? "), wsname, MAX_WS_NLEN );
		space_strip( wsname );
		}
...
nodep xwhere;		/* what stub we graft on */
{
	...
	if( ntype( what ) == N_LIBRARY && ntype( find_true_parent( where ) ) != N_PROGRAM ) {
		error( ER( 322, "Can only copy libraries to main level" ) );
		}
	}
#endif
...
nodep	_np;
{
	...
	if (type == N_DECL_ID) {
		...
		cursor = np;		/* eww, gross */
		s_kid_id(np, slookup(str_kid(0,np), CREATE, NULL, MOAN, NIL));
		s_ntype(np, N_ID);
		}
...
workspace *xws;
{
	...
	printt3("New program in workspace %lx name %s, work node %lx\n",
			(long)ws, work_name(ws), (long)work_node(ws) );
