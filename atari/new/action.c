...
preg1 nodep foundnode;	/* node where action was found */
{
	...
	if (act == 0) {
		...
		nt = act - EXPAND;
#ifdef DB_ACTION
		if (tracing) fprintf(trace, "expand node type %d\n", nt);
#endif
		exp_prod(nt);
	} else {
...
#ifdef DB_ACTION
	if (tracing) fprintf(trace, "build node %x\n", (int)nnode);
#endif
...
skip_to_next_stub()
{
	...
	do {
#ifdef DB_ACTION
		if(tracing) fprintf(trace, "cursor at %x\n", PCURS);
#endif
	...
	if (c_at_root(cursor)) {
#ifdef DB_ACTION
		if (tracing) fprintf(trace, "cursor at root, reset to %x\n",
			PCURS);
#endif
		cursor = holdcp;
	}
		...
		if (tracing) fprintf(trace, "cursor skipped to %x\n", PCURS);
#endif
}
...
{
#ifdef DB_ACTION
	if (tracing) fprintf(trace, "skip_down, cursor starts at %x stub not %d is %d\n", (int)cursor, not_a_stub(cursor), is_a_stub(cursor));
#endif
	if( not_a_stub(cursor) ) {
#ifdef DB_ACTION
	if (tracing) fprintf(trace, "skip_down, cursor type %d, kid_count %d, lookaheadchar %d\n", ntype(cursor), kid_count(ntype(cursor)), lookaheadchar);
#endif
...
#ifdef DB_ACTION
	if (tracing) fprintf(trace, "skip_down, cursor ends at %x\n", (int)cursor);
#endif
...
#ifdef DB_ACTION
	if(tracing) fprintf( trace, "New stub of type %d - %s\n",
		ce, classname(ce) );
#endif
	if (makeList)
