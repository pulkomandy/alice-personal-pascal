...
int islibrary;
{
	...
	BusyBee( TRUE );
#endif
	load_misc( islibrary );	/* loads flags, among other things */
...
int islibrary;
{
	...
	if( c == 'L' ) {
		...
		if( islibrary != LIBRARY )
			error( ER( 325, "Improper use of binary library" ) );
...
int islibrary;	/* Flag that says whether or not we are loading a library */
{
	...
	while ((type = getbyte(loadf)) == N_DECL_ID) {
		...
		if( sym_dtype( sp ) == T_LFUNC || sym_dtype(sp) == T_LPROC ){
			printt1( "Adjusting value of proc/func %s\n",
				sym_name( sp ) );
		...
		if( islibrary == LIBRARY || islibrary == MERGE ) {
			/* Loading an UNDEF symbol from within a library */

			if( sym_dtype( sp ) == T_UNDEF ) {
				extern symptr slookup();
				ptr = slookup( sym_name( sp ), CREATE, 
					GlobalSymtab, NOMOAN, sp ); 
				/* If a new symbol was created, then patch
				 * that into the table, but don't link it
			...
			printt1( "Loading library symbol %s\n", sym_name(sp));
			if( islibrary == LIBRARY )
...
nodep top;
{
	...
	for (index = listcount(nlist) - 1; /* test in middle */; index--) {
		...
		/*
		 * Now that it has been grafted, stomp on the pointer
		 * so that when we free everything, the list will
		 * be freed, but the decls won't
		 */
		s_node_kid( nlist, index, NIL );
	...
	/*
	 * Now stomp on the sym_kid of the program node so that that
	 * is not freed
	 */
	s_node_kid( top, kid_count(ntype(top)), NIL );
