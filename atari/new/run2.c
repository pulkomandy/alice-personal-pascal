...
int stepmode;
{
	...
	framesize sizes[MAX_SCOPE];		/* sizes of intermediate scopes */
	...
	if( ntype(loc) < FIRST_RUN || ntype(loc) > LAST_RUN || ntype(loc) == N_LIBRARY )
...
nodep xblock;
{
	...
	temp = (framesize)node_kid( block, kid_count(btype)+1 );
	EDEBUG( 8, "act_size( %lx ) = %ld\n", (long)xblock, (long)temp );
...
nodep xeloc;
{
	...
	if( (gkflags & NF_VALMASK) == NF_STPUSH || gkflags & NF_CHRSTR) {
		...
		EDEBUG( 5, "Old Stack Pointer: %lx\n", (long)sp.st_generic,0);
			bump_stack( size_adjust(formsize) );
		EDEBUG( 5, "New Stack Pointer: %lx\n", (long)sp.st_generic,0);

			EDEBUG( 4, "Pushing object at %lx size %ld onto stack\n",
					(long)tempptr, (long)formsize );
			/* check_undef( tempptr, formsize ); */
			/*do_undef( sp.st_generic, U_SET, formsize ); */
			copy_mask( sp.st_generic, tempptr, formsize );

			EDEBUG( 4, "Copying from %lx to %lx\n",
				(long)tempptr, (long)sp.st_generic );
	...
	for( dex = 0 ; !valid ; dex++ ) {
		...
		if(  valid == 0 )
			...
			if( filedesc->f_flags & FIL_LAZY ) {
				...
				{
				char pbuf[80];
				sprintf( pbuf, LDS( 230, 
		"Sorry, '%s' isn't a valid number, please try again"), badnum );
				pure_message( pbuf );
	...
	for( i = 0; i < whfile->f_size; i++ ) {
		...
		if (flstore & FIL_KEYBOARD ) {
			...
			if( flstore & FIL_RAWKEY ) {
				...
				if( sai_out == SAI_SCREEN ) {
#endif
				if( flstore & FIL_SCREEN && whfile->desc.f_window ) {
					curOn( whfile->desc.f_window );
...
#endif
					if( flstore & FIL_SCREEN && 
						whfile->desc.f_window )
						curOff( whfile->desc.f_window );
				}
			 else
 ...
 */
			...
			if( got_break() )
				return;
#ifdef TOS
			if( flstore & FIL_AUXDEV ) {
				c = auxget( whfile->desc.f_stream );
			} else
#endif TOS
...
#else
#ifdef GEM
	PgTitle( LDS( 192, " Input - F6 for EOF " ));
#else
...
string_add()
{
	...
	bump_stack( len2+1+align );	/* add one for the zero */
