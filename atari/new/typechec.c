/*
 * typecheck a subtree, and perhaps the whole program.
 *
...
static framesize asg_result;	/* compat code for assignment */
static NodeNum ret_type;	/* node type of return type */
static listp ourargs;	/* an argument list */

extern framesize ccheck_func();	/* case check function */
...
ClassNum theclass;		/* expected class for this subtree */
{
	...
	switch( treetype ) {
	 ...
	 case N_ST_CASE:
		...
		if( is_ordinal(kidt1) )
			scan_decl( FLCAST tkid2, ccheck_func, (framesize)0,
				FALSE, 0, (pint)comp_type(kidt1));
		 else
	 ...
	 case N_ST_ASSIGN:
		...
		printt2( "Setting type of assignment on %lx to %ld\n",
			(long)tree, (long)asg_result );
...
#else
	if( tracing ) fprintf( trace, "%s at %lx\n", getERRstr(string), (long)tree );
#endif SAI
...
nodep atype;
{
	...
	if( ttype == N_DECL_ID ) {
		...
		if( is_stringtype( otype ) )
			...
			return (framesize)0-(framesize)calc_size(atype,TRUE);
		}
}
...
framesize parm_no;		/* which parameter */
...
type_error(tree, ER(164,"fewargs`Too few arguments, should be at least %d"),
	(int) PTS(built_rot , min_params, realrout_np) );
			else if( listcount(arglist) > PTS(built_rot,max_params,realrout_np))
type_error(tree, ER(165,too_many), (int)PTS(built_rot,max_params,realrout_np));
			else if( PTS(built_rot, r_decls,realrout_np) )
				scan_decl( FLCAST PTS(built_rot, r_decls,
				 rout_node far *), argcheck_func, (framesize)0,
				 FALSE, 0, (pint)arglist );
			}
		}
		else if( thedec ) {
			static framesize formcount;
			formcount = scan_decl( FLCAST kid2(thedec), argcheck_func,
				(framesize)0, FALSE, 0, (pint)arglist );
