...
#define LOAD_ERRORS	1
#endif

#ifdef LOAD_ERRORS
/* Error messages, etc are loaded from a file */
# define ER(num,str)	num
# define LDS(num,str)   getLDSstr( num )
/* Get a language dependent string */
extern char *getLDSstr();
extern char *getERRstr();
#else
/* Error messages are compiled inline */
# define ER(num,str)	str
# define LDS(num,str)   str
...
#define long_kid(num,NdP)	((long)(NdP)->n_kids[num])
#define size_kid(num,NdP)	((asize)(NdP)->n_kids[num])
...
#define s_long_kid(num,NdP,val) s_node_kid(NdP,num,NCAST (val))
#define s_size_kid(num,NdP,val) s_node_kid(NdP,num,NCAST (val))
...
#define s_long_kid(num,NdP,val) ((NdP)->n_kids[num] = (nodep )(val))
#define s_size_kid(num,NdP,val) ((NdP)->n_kids[num] = (nodep )(val))
...
#define is_a_hide(NdP)	(*((bits8 *)NdP)==N_HIDE||*((bits8 *)NdP)==N_REVEAL||*((bits8 *)NdP)==N_LIBRARY)
#define is_a_revorhide(NdP)	(*((bits8 *)NdP)==N_HIDE||*((bits8 *)NdP)==N_REVEAL)
...
#define sym_offset(sNdP)	((framesize)(sNdP)->s_value)
