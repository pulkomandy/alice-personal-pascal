...
int do_estack;		/* flag indicating what is to be done with ex_stack */
{
	...
	if( !(exec_stack && loc_stack) ) {
		EDEBUG( 5, "exec_stack or loc_stack alloc failed\n",0,0);
...
/* 		newClear();	set up new segment */
	...
	EDEBUG( 3, "Set stack ptr to %lx, ex_stack to %lx\n", (long)sp.st_stackloc,
					(long)ex_stack );
...
framesize varsize;	/* how many bytes for variables */
...
int hiddenrot;			/* is this routine hidden */
{
	...
	EDEBUG( 3, "Creating new stack frame scope %d, varsize %ld\n", scope,
		(long)varsize );
	...
	EDEBUG( 5,"%ld bytes allocated in stack\n", (long)max_stack_size,0 );
	EDEBUG( 5,"%ld bytes left in stack\n", (long)sp.st_generic - (long)
			free_top,0);
	EDEBUG( 5,"need %ld bytes\n", (long)(
		(framesize)STACK_BUFFSZ + varsize + (framesize)indispsize +
		(framesize)(EXTR_DISP_WORDS * sizeof( pointer ))), 0 );
	EDEBUG( 5, "ex_stack = %lx, loc_stack = %lx\n", (long)ex_stack,
			(long)loc_stack );

	if(  (  (framesize)((long)sp.st_generic - (long)free_top) < 
	       ((framesize) STACK_BUFFSZ + 
		            varsize +
		(framesize) indispsize + 
		(framesize)(EXTR_DISP_WORDS*sizeof(pointer) )
	       )
	     )
	...
	* (pointer )newdisp = scope;	/* save away scope */
	...
	EDEBUG( 4, "Argument pointer this frame is %lx\n", (long)ourap, 0 );
	...
	/* NOTE: Bug in mwc 2.0 with using -= and += with pointers and longs */
	bump_stack(varsize);
		/* abort on run errors? */
	/* make sure strings have zero bytes */

#define ZER_INCR	32000

#ifdef MC68000
	if( varsize > (long)ZER_INCR ) {
		framesize counter;
		pointer wtc;

		counter = varsize;
		wtc = sp.st_generic;
		while( counter > (long)ZER_INCR ) {
			do_undef( wtc, U_CLEAR, (asize)ZER_INCR );
			zero( wtc, (asize)ZER_INCR );
			wtc = wtc + (long)ZER_INCR;
			counter = counter - (long)ZER_INCR;
			}
		do_undef( wtc, U_CLEAR, (asize)counter );
		zero( wtc, (asize)counter );
		}
	 else
#endif
	 {
	zero( sp.st_generic, (asize)varsize );
	/* now mark the variables as uninitialized */
	do_undef( sp.st_generic, U_CLEAR, (asize)varsize );
	}
}
...
asize needed;
{
	if( ((framesize)sp.st_generic - (framesize)free_top) <
			 (framesize)needed ) {
		run_error(  ER(105,stoverrs) );
		}
}
...
#ifdef GEM
	suspendGraphics();	/* If mouse is off, turn it on */
	resetVDI();		/* Reset the VDI */
	ErrAlert( str, a1, a2, a3, a4, a5 );
	ShowHidden();		/* Reveal hidden edit windows */
	TopCurEdit();		/* Top the current edit window */
#else
...
pointer to;
pointer from;
asize bytes;	/* how many bytes to move */
{
	register framesize todiff, fromdiff;	/* byte offsets */
	pointer mapto, mapfrom;			/* location in map */
	unsigned int tooffset, fromoffset;	/* offsets in bytes */
	unsigned int i;
	...
	if( (todiff = (framesize)to - (framesize)exec_stack) > (framesize)
			max_stack_size )
		return;

	/* if source is not on stack, just set dest as defined */
	mapto = (pointer)(undef_bitmap + (todiff >> 3 ));
	if( (fromdiff = (framesize)from - (framesize)exec_stack) > 
			(framesize)max_stack_size ) {
		do_undef( mapto, U_SET, bytes );
		return;
		}

	EDEBUG( 5, "copy_mask( to = %lx, from = %lx )\n", (long)to,
			(long)from );

	EDEBUG( 5, "copy_mask( todiff = %lx, fromdiff = %lx )\n",
		(long)todiff, (long)fromdiff );

	EDEBUG( 5, "copy_mask( bytes = %lx )\n", (long)bytes, 0 );

	EDEBUG( 5, "copy_mask( src bitmap = %lx, dst bitmap = %lx )\n",
		(long)(&undef_bitmap[fromdiff >> 3]),
		(long)(&undef_bitmap[todiff >> 3]) );
...
unsigned int bytes;	/* how many bytes to handle */
{
...
#ifdef XDEBUG
	if( ed_level > 11 )
		fprintf(etrace, "do_undef( loc=%lx, mode=%d, bytes=%u )\n",
...
#if defined(LARGE) && !defined( TOS )
	if( FP_SEG(loc) != SSeg  ||  (bytedif = loc - (pointer)exec_stack) > max_stack_size )
		return;		/* checking memory out of stack, string ? */
#else
#ifdef TOS
	bytedif = (long)loc - (long)exec_stack;
	if( bytedif > (long)max_stack_size ) return;
	if( bytedif < 0L ) return;
#else
	...
	if( bytes <= 2 && startbit < 7 ) {
		...
		if( (togo = (long)startbit + (long)bytes - 9L) >= 0L ) {
			while( (togo -= 8L) >= 0L )
				domask( maploc++, 0xff, setit );
			/* and now do last byte */
			domask( maploc, 0xff >> (-1 - (int)togo), setit ); 
			}
		}
}
...
int setit;		/* flag for checking */
{
	switch( setit ) {
		...
		case U_SET:
			*byte |= mask;
/*			EDEBUG( 4, "Setting byte @ %lx with %x\n",
				(long)byte, (int)mask );
 */
			break;
		case U_CLEAR:
...
# if (defined(unix) || defined(ATARI520ST)) && defined(LARGE)
seg_fillchar( loc, fillbyte, bytes )
pointer loc;
register char fillbyte;
unsigned int bytes;
...
asize bytes;			/* size for thing being pushed */
...
#ifdef SAI
		if( sai_out == SAI_SCREEN ) wrefresh( filedesc->desc.f_window );
#else
		wrefresh( filedesc->desc.f_window );
#endif SAI
}
