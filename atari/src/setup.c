/*
 * Setup for interpretation and stuff
 */
...
{
	if (tracing)
		fprintf(trace, "farcheck [line %d in %s]\n", line, filename);
}
#endif
...
#ifndef MC68000
#define lcheckalloc checkalloc
#else
extern char *lcheckalloc();
#endif
...
init_interp()
{
	...
	if( !max_stack_size ) {
		...
		if( mlcalc < 0 )
			max_stack_size = 1024;
		 else if ( mlcalc > 65000L )
			max_stack_size = 65000;
		 else
			max_stack_size = (stacksize)mlcalc;
		
		}
	if( !lc_stack_size ) {
		lc_stack_size = max_stack_size/8;
		if( lc_stack_size > 2000 )
			lc_stack_size = 2000;
	}

	printt2( "max stack size %d, lc_stack_size %d\n", max_stack_size,
					lc_stack_size );
	exec_stack = (stackloc *) lcheckalloc( max_stack_size );
#if defined(LARGE) && !defined( ATARI520ST )
	SSeg = (unsigned)((long)exec_stack >> 16);
#endif
	loc_stack = (estackloc *) lcheckalloc((stacksize)lc_stack_size*sizeof(estackloc) );

#ifdef UDEBUG
	undef_bitmap = (bits8 *) lcheckalloc( (stacksize)
		(1+max_stack_size/BITS_IN_BYTE ));
#endif
...
int ishide;		/* is a hidden block */
{
	...
	framesize cursize;	/* how much of the stack frame we have so far*/
	...
	framesize vard_comp(), cond_comp(), typd_comp(); /*compiling routines */
	int kc;
	...
	if( btype == N_DECL_PROC || btype == N_DECL_FUNC ) {
		...
		scan_decl( FLCAST kid2(block), vard_comp, (framesize)0, TRUE,
					NF_ERROR, (pint)scope );
		}
	...
	for( bigdec = 0; bigdec < listcount( dlist ); bigdec++ ) {
		...
		case N_DECL_VAR:
			...
			break;
		case N_DECL_TYPE:
			scan_decl( FLCAST kid1(dblock), typd_comp, 
				(framesize)0, TRUE, NF_ERROR, (pint)0 );
			break;
		case N_DECL_CONST:
			scan_decl( FLCAST kid1(dblock), cond_comp,
				(framesize)0, TRUE, NF_ERROR, (pint)0 );
			break;
		case N_DECL_LABEL:
...
#ifdef DB_TYPES
				if(tracing)fprintf(trace,"Setting type for function %x to %x\n",
						ourdec, functype );
#endif
	...
	s_node_kid( block, kc+1, NCAST cursize );
	...
	if( ntype(type_parent) == N_CONST_INIT ) {
		...
		if( not_a_stub( ctree ) ) {
			...
			if( tc_interactive & TC_INIT ) {
				...
				type_error( type_parent,
 ER(277,"bigstruct`Array much too large -- shrink the bounds")
			 		);
				 else {	/* mark undefined */
...
pint scope;		/* general argument, scope in this case */
{
	...
	if( ptype == N_FORM_REF && is_a_stub(kid2(type_parent)) ) {
		...
		newsize = (framesize) sizeof(struct anyvar);
		}
	 else {
		...
		if( ptype == N_FORM_PROCEDURE )
			...
			newsize = (framesize)
				(calc_size(ptype == N_FORM_FUNCTION ?
				kid3(type_parent) : kid2(type_parent), FALSE ));
			}
		/* different amounts of memory are needed for different 
		 * kinds of parms
		 */
		s_sym_type(thedecl,find_realtype( NCAST thedecl ));
		}
	s_sym_size(thedecl,(asize)newsize);
	if( ptype == N_FORM_PROCEDURE || ptype == N_FORM_FUNCTION )
		newsize = (framesize)(sizeof(pointer) + sizeof(nodep ));
		/*passed proc */
	 else if( ptype == N_FORM_REF ) {
		newsize = (framesize)sizeof(pointer);
...
# else
			...
			type_error( ER(327,"Segmented absolute address not allowed in small model ALICE") );
# endif
		}
	 else
		{
		so_far += size_adjust( (asize)newsize );
		s_sym_offset(thedecl, so_far - WB_ADJUST( (asize)newsize ) );
		printt3( "vard_comp( sym = %lx ) - size = %ld, offset = %ld\n",
			(long)thedecl,
			(long)size_adjust( (asize)newsize ),
			(long)so_far );
