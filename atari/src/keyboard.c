/*
 * Keyboard input routines, called by scanner and editor.  These routines
 * handle keyboard macros, which do simple character substitution.
 */
...
char *str;
{
	...
	d = macbuf + strlen(str);
	if (d != macp)
		while (*d++ = *macp++)	/* copy trailing null */
			;
	d = macbuf;
	while (*str)
		*d++ = *str++;	/* don't copy trailing null */
	macp = macbuf;
}
...
char *args;
{
	...
	if (args && (p = strchr(args, ' '))) {
		*p++ = 0;
...
#ifdef GEM
		/*
		 * Put up a little dialog box, asking the user for the key
	 	 */
		MacroPrompt();
		MenuCtrl( FALSE );
#else
		...
		getprline( LDS( 439, "Expansion: "), buf, sizeof(buf) );
...
#ifdef TOS
/*
 * Push a key into the keyboard buffer stream
 */
push_key( key )
unsigned int key;
{
	if( inbuf_count < KBUF_SIZE )
		incom_buf[inbuf_count++] = key;
}
#endif
...
keyget()
{
...
#ifdef ATARI520ST
	/*
	 * If any keys have been queued, return them
	 */
	HadKeyEvent = FALSE;
	if( inbuf_count ) {
		gotch = incom_buf[0];
		printt2( "Returning queued char %d, %d left\n",
			gotch, inbuf_count );
	...
	gotch = GemEvent();
	printt1( "Returning GemEvent() of %d\n", gotch );
	return gotch;
#else 
