...
#ifdef UDEBUG
	if( arg >= FLOATNUM(32768.0) || arg <= FLOATNUM(-32768.0) )
...
#ifdef UDEBUG
	if( arg >= FLOATNUM(32767.5) || arg <= FLOATNUM(-32767.5))
		run_error( ER(31,intout), arg );
#endif UDEBUG
...
#ifdef TOS
		*argv = IntCheck( (long)num * (long)num, LDS( 419, "square"));
#else
		*argv = num * num;
#endif
		}
...
pointer *argv;
{
	...
	checksvar( NCAST argv[-1], LDS( 420, "StrConcat first argument" ));
...
symptr sym;
{
	...
	st1 = argv[-2] + STR_START;
	checksvar( NCAST argv[-1], LDS( 421, "String Delete string" ));
...
symptr sym;
{
	...
	checksvar( dest_type, LDS( 422, "String Insert string" ));
	/* remove the type */
	stlen = strlen( st1 );
...
pointer *xargv;
{
	...
	st2 = str_spop() + STR_START;
	checksvar( dest_type = NCAST p_spop(), LDS( 423, 
				"SubStr destination string" ));
	off_set = int_spop() - 1;
...
rint *argv;
{
	...
	if( argc )
		*argv = (rint)(((long)rand() * (long)int_stop)/ 32768L);
	 else if( turbo_flag )
		*(rfloat *)argv = rand() / 32768.0;
		...
		if( rand_modulus > 0 )
			*argv = (rint)(((long)rand() * rand_modulus)/32768L);
	 	else
...
int dir;	/* direction */
{
	...
	if( argc >= 3 )
		...
		seg = 0; /* Very Dangerous !!! */

	temp = (pointer)(((long)seg << 16) + off);

	/* large model ? */
	if( dir )
		blk_move( temp, varadr, blksize );
	 else {
		blk_move( varadr, temp, blksize );
...
#endif
#else
	run_error( ER(0,"unfinished`Block moves not implemented here") );
#endif
}
#endif MC68000
...
rint *argv;
{
	...
	if( value )
		Giaccess( 0xfe, 0x87 );	/* Channel A active only */
	else {
		Giaccess( 0xff, 0x87 ); /* All channels off */
		return;
	}

	Giaccess( 0x0f, 0x88 );	/* Default volume */
	Giaccess( 0x00, 0x8b ); /* Attack, decay */
	Giaccess( 0xff, 0x8c ); /* Sustain, Release */

	freq = value;

	period = 125000L / (long)freq;

	Giaccess( period & 0xff, 0x80 );
