...
nodep xfoundnode;/* where the action was found */
{
	...
	if (tracing) fprintf(trace, "action routine #%d, token %d\n", arn, tn);
#endif
	switch(arn) {
...
#if defined(DB_ACTROUT)
	if( tracing )
		fprintf( trace, "Sticking symbol table on %x at child %d\n",
...
#ifdef DB_ACTROUT
	if (tracing) fprintf(trace, "decl_it dtype %d, lookup %x, looktype %d, lp %x\n", dtype, lookup, looktype, lp);
#endif
