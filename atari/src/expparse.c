...
#ifdef DB_EXPPARSE
#define checkrhs(num) if (rhssize() != num) { if (tracing) fprintf(trace, "checkrhs %d fails, found %d\n", num, rhssize()); return TRUE; }
#else
...
stackdump()
{
	...
	if (trace == NULL)
		return;
	fprintf(trace, "stack(%d): ", stacktop);
	for (bot=0; bot<=stacktop; bot++) {
		...
		if (btn < E_EXP) {
			if (bot > 0) {
				...
				case EQ: fprintf(trace, " ="); break;
				case LT: fprintf(trace, " <"); break;
				case GT: fprintf(trace, " >"); break;
				case RR: fprintf(trace, " ?"); break;
				case FIN: fprintf(trace, " ."); break;
				}
			}
			ptn = btn;
		}
		fprintf(trace, " %s(%x)", opname(btn), btt);
	}
	fprintf(trace, "\n");
}
