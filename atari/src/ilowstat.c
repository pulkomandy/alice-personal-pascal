	/*
	 *
	 *   SPECIAL FILE OF CASES FOR INFREQUENT INTERPRETER STATES
	 *   for use with dumb compilers that can't handle really long
	 *   functions
	 *
	 */
...
#endif TURBO
#ifdef WORD_ALLIGNED
			if( sym_size(thesym) == 1 )
				check_undef( sp.st_generic + 1, 1 );
			else
#endif
			 ...
			 popadr:
				...
				*(bits8 *)tempptr = temp2;
				...
				if( step_flag & STEP_SINGLE  && !within_hide) {
					extern char *ordname();
					nonum_message( LDS(433,"Scalar := %s"),
...
#ifndef SAI
				if( step_flag & STEP_SINGLE && !within_hide )
					nonum_message( LDS(434,"Integer := %d"), temp2 );
#endif
...
#ifndef SAI
				if( step_flag & STEP_SINGLE  && !within_hide)
					nonum_message( LDS(435,"Real := %g"), fltemp );
#endif
	...
	/* PUT IN A CALL TO ORDNAME HERE */
	 ...
	 case Main(N_ST_TRACE):
		traceon( p_spop(), calc_size( gexp_type(kid1(eloc)), TRUE ) );
		break;
