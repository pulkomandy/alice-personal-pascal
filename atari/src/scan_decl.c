...
framesize (*procfun)();		/* thing to do with it */
framesize total;			/* running tally */
				...
				/* MUST BE BIG ENOUGH TO HOLD POINTER */
{
	...
	for( count1 = 0; count1 < listcount( dlist ); count1++ ) {
		...
		/* it is safe not to go down a library here because there
		   simply can't be one at this sort of level.  I don't know
		   if there needs to be such a rule, but the only thing it
		   disallows would be libraries of things like field decls */
		 else if( is_a_revorhide(sub_el) ) {
			/* recur */
...
symptr	sp;
{
	...
	framesize		nParams;	/* number of params */
	rout_np rp;		/* ptr to builtin's info */
	framesize		countParams();
	listp sized_list();

	if ( strchr( get_stubs, sym_dtype(sp) ) != 0  )
		nParams = scan_decl(FLCAST kid2(tparent(sp)), countParams,
			(framesize)0, FALSE, 0, (pint)0);
	else if ((rp = (rout_np )tparent(sp)) != NIL)
		nParams = PTS(rp, min_params, rout_node far * );
	else
		nParams = 1;

	lp = sized_list((int)nParams);
...
{
#ifndef SAI
	extern framesize argp_func();
	stackcheck();
	printt1("argp_call, plist=%lx\n", plist);
	return scan_decl( plist, argp_func, (framesize)0, FALSE, 0, thenode );
#endif SAI
}
#endif
