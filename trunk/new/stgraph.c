...
int HAspectRatio[] = { 9, 9, 9 };
int WAspectRatio[] = { 8, 4, 8 };

int WCurAspect;	/* The current aspect ratio */
int HCurAspect;
 ...
 */
		case CO_NORMAL:
			size = min( workrect.g_w * WCurAspect,
				     workrect.g_h * HCurAspect );
			height = size / HCurAspect;
			width  = size / WCurAspect;
			dwidth = dheight = 32767L;
			break;
		case CO_REVNORMAL:
			size = max( workrect.g_w * WCurAspect,
				    workrect.g_h * HCurAspect );
			height = size / HCurAspect;
			width  = size / WCurAspect;
			dwidth = dheight = 32767L;
...
int *y;
{
	...
	switch( _currSystem ) {
		...
		case CO_PIXEL:
			return;
		case CO_NORMAL:
			size = min( scr_area.g_w * WCurAspect,
				     scr_area.g_h * HCurAspect );
			height = size / HCurAspect;
			width  = size / WCurAspect;
			dwidth = dheight = 32767L;
			break;
		case CO_REVNORMAL:
			size = max( scr_area.g_w * WCurAspect,
				    scr_area.g_h * HCurAspect );
			height = size / HCurAspect;
			width  = size / WCurAspect;
			dwidth = dheight = 32767L;
...
resetGraphics()
{
	...
	WCurAspect = WAspectRatio[ Getrez() ];
	HCurAspect = HAspectRatio[ Getrez() ];
