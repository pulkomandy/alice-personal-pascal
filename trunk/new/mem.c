...
#ifdef MarkWilliams
	if (p)
		MemUsed += (size + (size & 1)) + sizeof( long );
#else
	if (p)
...
#ifdef MC68000
/*
 * Special LONG allocator for stacks, etc.  Called from 'setup.c'
 */
char *
lcheckalloc( size )
register long size;
{
	...
	register char *m;
	extern char *lmalloc();
	/* Note: Change this to be more 'checkalloc' ish */

	p = lmalloc( size );
	/* If it was successfully allocated, adjust MemUsed and
	 * zero out the memory
	 */
	if (p) {
		MemUsed += (size + (size & 1L)) + sizeof( long );
		m = p;
		while( size-- ) *m++ = 0;
	} else
		memerr();
...
nodep	_np;
{
	...
	/* If not ES, then fall through and handle like other strings */
	case N_NAME:
	case N_CON_STRING:
...
#ifdef ES
		freeESString( str_kid(2, np ) );
#else
		dmfree( str_kid(2, np ) );
#endif
		/* free the declarations */
		/* done above ... treefree( kid2(np) ); */
		/* INSERT CODE HERE TO FREE THE CODE BLOCK */
		freelib( kid4(np) );
