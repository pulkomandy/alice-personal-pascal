...
# else
#  ifdef GEM
#    define set_attr( attr ) chmask = (chmask & (A_BOLD|A_UNDERLINE|A_REVERSE|A_DOTUNDERLINE)) | attr;
#  else
...
/* 
   ...
   */
#define astandout() chmask |= _STANDOUT
		...
		if( *temp_ptr == '\t' )
			...
			case 'P':	/* parameter list */
			 ...
			 case 'c' : /* cursor is here */
				...
				printt2( "lb_cur = %ld dec, linbuf = %ld dec\n",
				...
				if( setecol ) {
					printt1( 
			"setecol true, set srch_real to %d\n", srch_real );
...
listp plist;	/* actual parameter list */
{
	...
	treeprint( node_kid( plist, (int)parm_no ), 0, ',', def, LNIL );
	return ++parm_no;
}
