...
#ifdef GEM
	getprline( LDS( 478, "Command:" ), holdbuf, MAX_LEN_LINE );
#else
...
#ifdef IBMPC
#define PROMPTC AC_WHITE|A_REVERSE
#else
# ifdef ICON
...
int maxlen;		/* max length of input */
{
	...
	if(!macp) {
#ifdef GEM
		GemCommand( prompt, inbuf );
#else
		promptBot(prompt);

		charToBE(prompt, beprompt,PROMPTC);

		oledit(curWindows[CMDWIN], 0, OLE_NOHL|OLE_CMD, beprompt,
			 BENULL, BENULL, (char *)NULL, inbuf, 0);
		statusLine();
#endif
...
buf_el *xcp;	/* ptr to end of area we want, beg was in cp to save_ctext */
{
	...
	*savep++ = 0;
	...
	if (tracing) {
		...
		fprintf(trace, "save_ftext, saveindent %d, len %d, clbuf -",
			saveindent, savep - clbuf);
		for (i=0; clbuf[i]; i++)
			fprintf(trace, "%x ", clbuf[i]);
		fprintf(trace, "-\n");
	}
#endif
}
