...
ISTAT rint temp2;			/* two quick temporaries */
					/* temp1 a register */
ISTAT asize sizetemp;			/* temp for sizes */
...
# ifdef UDEBUG
			if( temp1 < 0 || temp1 > int_kid( 1, type_ptr ) )
				run_error( ER(60,runerrstr), ordname(Basetype(
					BT_integer), temp1 ) );
# endif
			do_pushing( str_spop() + temp1, (asize)1, Basetype(BT_char) );
			break;
			}
#endif TURBO
		sizetemp = size_kid( 2, type_ptr );
#ifdef FULL
...
# endif
			...
			if( sizetemp <= 2 ) {
				do_pushing( p_spop()+(sizetemp == 2 ?
					((long)temp1<<1):(long)temp1), sizetemp,
					gexp_type(eloc));
				}
			 else
				do_pushing( p_spop()+((long)sizetemp*(long)temp1),
					sizetemp, gexp_type(eloc) );
			}
		}
		break;
...
# endif
...
#ifdef TOS
			long result;
			result = (long) int_spop();
			result += (long) int_stop;
			int_stop = IntCheck( result, LDS( 437, "addition" ));
#else TOS
			temp1 = int_spop();
#ifdef QNX
	 ...
	 case Main(N_EXP_MINUS):
		...
		if (type_ptr == Basetype(BT_integer)) {
			...
			long result;
			result = (long)int_spop();
			result = (long) int_stop - result;
			int_stop = IntCheck( result, LDS( 454, "subtraction"));
#else TOS
			temp1 = int_spop();
#ifdef QNX
	 ...
	 case Main(N_EXP_TIMES):
		...
		if (type_ptr == Basetype(BT_integer)) {
			...
			result = (long) int_spop();
			result *= (long)int_stop;
			int_stop = IntCheck( result, LDS( 438, 
					"multiplication" ));
#else TOS
			temp1 = int_spop();
#ifdef QNX
		...
		switch( sym_dtype(thesym) ) {
			case T_VAR:
				tempptr = (pointer)
					...
					sym_offset( thesym ));
				EDEBUG(8,
 "Variable from scope %d gives address %lx\n", thescope, (long)tempptr );
				break;
...
#if !defined(ASM_ICORE) || defined(TOS)
	if( got_break() ) {
		breakkey();
		clear_break();
#ifndef UDEBUG
...
/*
 * If runtime debugging is enabled:
 */
...
#ifdef TOS
int
IntCheck( result, cause )
register long result;
char *cause;
{

	if( result < -32768L || result > 32767L )
		run_error( ER(63, "integer %s overflow" ), cause );
