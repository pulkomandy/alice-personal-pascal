...
#ifdef DB_OUTFUNCS
if (tracing) {
	fprintf(trace, "in go_to_prev(%x) ", line);
	fprintf(trace, "with line->listptr=%x, line->listindex=%x, line->sub_line=%x, line->pcode=%d\n",
...
alice_window *xwin;		/* window to do this in */
{
	...
	if (tracing) {
		...
		fprintf(trace, "in top_cup with win=%x,mainwin=%x\n",win,curWindows[MAINWIN]);
		fprintf(trace, "win->start_cursor=%x\n", win->start_cursor);

		for (i = 0; i < win->w_height; i++)
			fprintf(trace, "w_lines[%d]->listptr = %x, w_lines[%d]->listindex = %d\n", i, win->w_lines[i].listptr, i, win->w_lines[i].listindex);
	}
#endif

	scrl_copy( topline, win->w_lines[0] );
	sub = topline.sub_line;
	MvSlider = FALSE;

#ifdef DB_OUTFUNCS
	if (tracing) fprintf(trace, "topline.listptr=%x, topline.listindex=%d, topline.sub_line=%d\n",  topline.listptr, topline.listindex, topline.sub_line);
#endif
	if( go_to_prev( &topline ) ) {
		if( topline.sub_line != (sub - 1) )
			MvSlider = TRUE;
#ifdef DB_OUTFUNCS
		if (tracing) fprintf(trace, "topline.listptr=%x, topline.listindex=%d, topline.sub_line=%d\n",  topline.listptr, topline.listindex, topline.sub_line);
#endif
...
#ifdef DB_OUTFUNCS
		if (tracing) fprintf(trace, "win->start_cursor = %x\n", win->start_cursor);
#endif
...
#ifdef DB_OUTFUNCS
		if (tracing) fprintf(trace, "go_to_prev returned false\n");
#endif
		return(FALSE);
	}
}
...
alice_window *xwin;
{
	...
	/* get a copy of the last line of the screen */
	scrl_copy( lastline, win->w_lines[win->w_height-1] );
	sub = win->w_lines[0].sub_line;
	MvSlider = FALSE;	/* Say we should move the slider */
	if( go_to_next( &lastline, lastline.sc_size, lastline.sub_line ) ) {
		/*
		 * If we are on the same physical line, then don't move the
		 * slider
		 */
		...
		/* If we moved onto a different line, then say we should
		 * move the slider
		 */
		if( sub != (win->w_lines[0].sub_line - 1) )
			MvSlider = TRUE;
...
find_again:
	...
	if( i >= 0 ) {
		...
		if( pc_index ) {
			...
			cur_line = i;
#ifdef DB_OUTFUNCS
			if(tracing)fprintf(trace,"Looking for %x on line %d\n", PCP(srch_cursor), i );
#endif
	...
	/* search down for it */
#ifdef DB_OUTFUNCS
	if(tracing)fprintf(trace, "Scanning down for cursor line\n" );
#endif
...
#ifdef DB_OUTFUNCS
				if(tracing)fprintf(trace, "Found it %d lines down\n", i);
#endif
				while( i-- >= 0 )
					if( dir )
						bot_cdown(win);
...
int *lchild;		/* what child we came up from */
{
	...
	if( lchild )
		*lchild = -1;
#ifdef DB_OUTFUNCS
	if(tracing)fprintf(trace,"Asked to find line of %x\n", PCP(curs) );
#endif
	...
	if( oldkid != NIL ) {
		...
		if(lchild)
			*lchild = -1;
#ifdef DB_OUTFUNCS
		if(tracing)fprintf(trace,"Can't find %x in %d kids of %x\n", oldkid, imax, ascend );
#endif
