...
#ifdef MarkWilliams
/* MWC 2.0 brain-damaged - can't use += or -= with longs and pointers */
#define bump_stack(val) (sp.st_stackloc = sp.st_stackloc - (val))
#define unload_stack(val) (sp.st_stackloc = sp.st_stackloc + (val))
#else
...
extern stacksize max_stack_size;
extern stacksize lc_stack_size;
